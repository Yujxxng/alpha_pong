...
void Levels::GameOver::Update()
{
	...
	AEGfxPrint(fontName0, "GAME OVER!", -width / 2, -height / 2 + 0.55f, 1.f, 1.f, 1.f, 1.f, 1.f);

	AEGfxGetPrintSize(fontName0, "*Your Score*", 0.8f, &width, &height);
	AEGfxPrint(fontName0, "*Your Score*", -width / 2, -height / 2 + 0.35f, 0.8f, 1.f, 1.f, 1.f, 1.f);

	AEGfxGetPrintSize(fontName0, to_string(TotalScore).c_str(), 1.f, &width, &height);
	AEGfxPrint(fontName0, to_string(TotalScore).c_str(), -width / 2, -height / 2 + 0.2f, 1.f, 0.f, 1.f, 0.f, 1.f);

	std::string tmp = "Highest score : " + to_string(HighScore);
	AEGfxGetPrintSize(fontName0, tmp.c_str(), 0.5f, &width, &height);
	AEGfxPrint(fontName0, "Highest score : ", -width / 2 - 0.05f, -height / 2, 0.5f, 1.f, 1.f, 0.f, 1.f);

	AEGfxGetPrintSize(fontName0, to_string(HighScore).c_str(), 0.6f, &width, &height);
	AEGfxPrint(fontName0, to_string(HighScore).c_str(), -width / 2 + 0.25f, -height / 2, 0.6f, 1.f, 1.f, 0.f, 1.f);
